...
export function PantryCheckView({ 
  ...
  const displayDepts = useMemo(() => {
    return localDepts.filter(dept => {
      ...
      // חוק ברזל: אם אין מוצרים במחלקה הזו, לא מציגים אותה בשום מצב!
      if (items.length === 0) {
        return false;
      }
      
      // אם יש חיפוש - מציגים רק אם שם המחלקה או שם המוצר מתאימים לחיפוש
      if (searchQuery) {
      ...
      // אם אין חיפוש (ויש מוצרים) - מציגים כרגיל
      return true;
    });
                  ...
                  <SortableDepartmentItem 
                    ...
                    renderCard={(dragHandle) => (
                      <Collapsible 
                        ...
                        className={`bg-white rounded-2xl shadow-sm border border-slate-300 border-r-8 ${config.border}`}
                      >
          ...
          <DragOverlay dropAnimation={{ sideEffects: defaultDropAnimationSideEffects({ styles: { active: { opacity: "0.4" } } }) }}>
            {activeDept && activeDeptConfig ? (
              <div className="w-full flex justify-center opacity-100 drop-shadow-2xl">
                <div className="w-full max-w-[calc(100vw-20px)] md:max-w-[calc(100vw-32px)]">
                  <div className={`bg-white rounded-2xl shadow-sm border border-slate-300 border-r-8 ${activeDeptConfig.border}`}>
