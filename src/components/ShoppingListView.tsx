...
// מאגר 20 צבעים ייחודיים - מסונכרן לחלוטין עם PantryCheckView
const COLORS = [
  { borderClass: 'border-r-red-500', iconClass: 'text-red-500' }, // קצביה
  { borderClass: 'border-r-green-500', iconClass: 'text-green-500' }, // ירקות
  { borderClass: 'border-r-blue-500', iconClass: 'text-blue-500' }, // מקרר/חלב
  { borderClass: 'border-r-orange-500', iconClass: 'text-orange-500' }, // מזווה
  { borderClass: 'border-r-purple-500', iconClass: 'text-purple-500' }, // פארם
  { borderClass: 'border-r-cyan-500', iconClass: 'text-cyan-500' }, // דגים
  { borderClass: 'border-r-pink-500', iconClass: 'text-pink-500' }, // פירות
  { borderClass: 'border-r-amber-500', iconClass: 'text-amber-500' }, // מאפייה
...
export function ShoppingListView({
  ...
  // מנוע הקצאת צבעים ייחודיים ללא כפילויות (זהה למלאי)
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-slide-in">
        <div className="bg-white p-8 rounded-3xl border border-dashed shadow-sm flex flex-col items-center text-center">
          ...
          <p className="text-muted-foreground mt-1">אין מוצרים שחסרים כרגע</p>
        </div>
      </div>
    );
  }
        ...
        <div className={`relative bg-gradient-to-br from-indigo-600 via-indigo-700 to-violet-800 shadow-xl overflow-hidden transition-all duration-300 ease-in-out ${
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div className="relative w-full">
              ...
              />
              {isSearching && (
                <button onClick={() => setSearchQuery("")} className="absolute inset-y-0 left-0 flex items-center pl-3 text-indigo-200">
                  <X className="h-5 w-5" />
                </button>
              )}
            </div>

            {/* התקדמות וכפתורים - נעלמים בחיפוש כדי לפנות מקום למקלדת */}
            {!isSearching && (
              <div className="space-y-5 animate-in fade-in zoom-in-95 duration-300">
                <div className="space-y-2 px-1">
                  ...
                  <div className="h-3 w-full bg-black/10 rounded-full overflow-hidden shadow-inner">
                    <div 
                      className="h-full bg-gradient-to-r from-emerald-400 to-teal-300 transition-all duration-700 rounded-full shadow-[0_0_10px_rgba(52,211,153,0.3)]" 
                ...
                <div className="flex gap-3">
                  <Button onClick={onCopyList} variant="outline" className="flex-1 gap-2 rounded-2xl h-14 bg-white/5 border-white/20 hover:bg-white/10 text-white font-bold">
                    <Copy className="h-5 w-5" /> העתק
                  </Button>
                  
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button variant="default" className="flex-1 gap-2 rounded-2xl h-14 bg-white text-indigo-700 hover:bg-indigo-50 shadow-lg font-black" disabled={isFinishing || checkedCount === 0}>
                        <CheckCircle2 className="h-5 w-5" /> סימנתי!
                      </Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent className="rounded-3xl p-6">
                      <AlertDialogHeader>
                        <AlertDialogTitle className="text-right text-xl">סיימת לקנות?</AlertDialogTitle>
                        <AlertDialogDescription className="text-right text-base mt-2 font-medium">המערכת תמלא את המלאי עבור {checkedCount} המוצרים שסימנת.</AlertDialogDescription>
                      </AlertDialogHeader>
                          ...
                          >
                            <div className="flex items-center gap-5">
                              <div className={`w-7 h-7 rounded-full flex items-center justify-center border-2 transition-all duration-500 ${
                                isChecked ? "bg-indigo-500 border-indigo-500 text-white" : "bg-white border-slate-200 group-hover:border-indigo-300"
                              }`}>{isChecked && <Check className="h-4 w-4" strokeWidth={4} />}</div>
                              ...
                              <div className="flex flex-col text-right">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  <span className={`text-sm font-black ${isChecked ? "text-slate-300" : "text-indigo-600/80"}`}>{qty} {p.unit || "יחידות"}</span>
