  ...
  return (
    <div ref={setNodeRef} style={style} className={`flex items-center justify-between p-3 bg-card border rounded-xl shadow-sm group ${lactoseFree ? "border-sky-100 bg-sky-50/20" : ""}`}>
      ...
      <div className="flex items-center gap-2 px-2 border-x border-slate-100 mx-2">
        <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" onClick={() => onUpdateStock(product.id, Math.max(0, product.current_stock - 1))}><Minus className="h-3 w-3" /></Button>
        <span className="w-4 text-center font-black text-sm">{product.current_stock}</span>
        <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" onClick={() => onUpdateStock(product.id, product.current_stock + 1)}><Plus className="h-3 w-3" /></Button>
      </div>
